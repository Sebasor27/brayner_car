...
if (isset($_POST['add_member'])) {
  ...
  if ($img_r == 'inv_img') {
    ...
    $q = "INSERT INTO `images_cr`(`name`, `picture`) VALUES (?,?)";
...
if (isset($_POST['get_members'])) {
  $res = selectALL('images_cr');
  while ($row = mysqli_fetch_assoc($res)) {
    $path = ABOUT_IMG_PATH;
    echo <<<data
       <div class="col-md-2 mb-3">
            <div class="card bg-dark text-white">
              <img src="$path$row[picture]/IMG_54224.jpg" class="card-img">
...
if (isset($_POST['rem_member'])) {
  ...
  $pre_q = "SELECT `picture` FROM `images_cr` WHERE `sr_no` = ?";
  $res = select($pre_q, $values, 'i'); 
  $img = mysqli_fetch_assoc($res);

  if ($img && deleteImage($img['picture'], ABOUT_FOLDER)) {
    $q = "DELETE FROM `images_cr` WHERE `sr_no` = ?";
